class MolecularProfileTsvFormatter
  ...
  def self.row_from_object(mp)
    [
      ...
      mp.variants.map {|variant| LinkAdaptors::Variant.new(variant).permalink_path(include_domain: true) },
      mp.evidence_score,
      mp.evidence_items.map(&:id).join(', '),
      mp.evidence_items.map {|eid| LinkAdaptors::EvidenceItem.new(eid).permalink_path(include_domain: true) },
      mp.assertions.map(&:id).join(', '),
      mp.assertions.map {|aid| LinkAdaptors::Assertion.new(aid).permalink_path(include_domain: true) },
