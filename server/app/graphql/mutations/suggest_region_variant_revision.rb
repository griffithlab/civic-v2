class Mutations::SuggestRegionVariantRevision < Mutations::MutationWithOrg
  description "Suggest a Revision to a Variant entity."
  argument :id, Int, required: true,
    description: "The ID of the Variant to suggest a Revision to."

  argument :fields, Types::Revisions::RegionVariantFields, required: true,
  ...
  field :variant, Types::Variants::RegionVariantType, null: false,
  ...
  def ready?(organization_id: nil, id:, fields:, **kwargs)
    ...
    if !variant.is_a?(Variants::RegionVariant)
      raise GraphQL::ExecutionError, "Variant with id #{id} is a #{variant.type} and you called the RegionVariant mutation."
    end
  ...
  def resolve(fields:, id:, organization_id: nil, comment: nil)
    updated_variant = InputAdaptors::RegionVariantInputAdaptor.new(variant_input_object: fields).perform
    updated_variant.feature = variant.feature
    updated_variant.region = variant.feature.feature_instance
    updated_variant.name = variant.name
    updated_variant.iscn_name = variant.iscn_name
