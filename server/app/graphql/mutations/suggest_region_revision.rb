class Mutations::SuggestRegionRevision < Mutations::MutationWithOrg
  description 'Suggest a Revision to a Feature entity of instance type "Region".'

  argument :id, Int, required: true,
    description: 'The ID of the Feature of instance type "Region" to suggest a Revision to.'

  argument :fields, Types::Revisions::RegionFields, required: true,
  ...
  field :region, Types::Entities::RegionType, null: false,
    description: "The Region the user has proposed a Revision to."
  ...
  def ready?(organization_id: nil, id:, fields:, **kwargs)
    ...
    region = Feature.find_by(id: id)&.feature_instance
    if region.nil?
      raise GraphQL::ExecutionError, "Feature with id #{id} doesn't exist."
    elsif !region.is_a?(Features::Region)
      raise GraphQL::ExecutionError, "Feature with id #{id} is not a Region feature."
    end
  ...
  def resolve(fields:, id:, organization_id: nil, comment:)
    updated_region = InputAdaptors::RegionInputAdaptor.new(region_input_object: fields).perform
    revised_objs = Activities::RevisedObjectPair.new(existing_obj: region, updated_obj: updated_region)

    cmd = Activities::SuggestRevisionSet.new(
      revised_objects: revised_objs,
      subject: region.feature,
