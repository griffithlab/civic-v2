...
# Guards the RecordLoader batching on the by-id entity resolvers: every id
# requested in one execution resolves in one `WHERE id IN (...)`. Swapping a
# loader back to `find_by` returns the same data with N queries instead of 1,
# so these tests count SQL. A resolver added to `query_type.rb` without a
# loader is the regression to catch.
