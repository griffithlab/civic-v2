...
# Guards that `evidenceItems(therapyInteractionType:)` filters. A filter that
# reaches no variable changes no rows, which from the client looks like one
# matching everything. evidence-manager.config.spec.ts guards the client half
# (every filterable column reaches a declared variable); this guards the
# server half.
