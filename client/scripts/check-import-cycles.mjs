 ...
 *
 * Reads the esbuild metafile from a `--stats-json` build, so the graph checked
 * is the one that ships, with aliases and `import type` erasure already
 * resolved. Only `import-statement` edges count: `import()` is evaluated on
 * demand and is how lazy routes load.
 *
 * Modules in a cycle can't be tree-shaken apart, so importing one drags in
 * all of them, and module-init order starts to matter (`undefined` at import
 * time).
 *
 * Usage:
