// Strips ng-zorro rule families the app never renders. An entry.less emits
// every sub-family (grid push/pull/offset/order across six breakpoints, the
// ghost button palette) whether or not a template sets the driving input.
// Sibling of postcss-strip-rtl.cjs; both load via postcss.config.json.
//
// ng-zorro applies these classes only from directive inputs (`nzPush` writes
// `.ant-col-push-N`, `nzGhost` writes `.ant-btn-background-ghost`), so no
// template setting the input means no element gets the class.
// `assertStillUnused()` rescans src/ during the build and fails if a driving
// input reappears; the fix is then to delete that entry from REMOVALS.
