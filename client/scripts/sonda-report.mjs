 ...
 *
 * Calls Sonda's core directly because `sonda-angular` looks for stats under
 * `angular.json`'s `outputPath`, `../server/public`, which holds tracked
 * deploy artifacts a local build must never clobber. `yarn build:stats`
 * writes to `dist/stats-build` instead.
 *
 * Usage:
