 ...
 * produced them. The esbuild metafile treats `src/styles.less` as one opaque
 * input, so this parses the compiled CSS and attributes each rule by selector;
 * `--families` splits an entry into rule families.
 *
 *   - Entries pull their own style dependencies (`table` brings `pagination`),
 *     so an entry's weight isn't what deleting its @import saves. Measure a
 *     removal with `--diff`; never predict it.
 *   - Attribution matches `.ant-<root>` prefixes against ROOTS below, because
 *     class names don't match package names (`date-picker` emits
 *     `.ant-picker`). Unmatched rules are reported under their own first
 *     segment, so drift stays visible.
 *
 * Usage:
...
/**
 * Sub-component rule families, each tied to a template input the app either
 * uses or doesn't. First match wins, so a rule spanning two families is
 * charged whole to the first: the figures are upper bounds, not savings.
 * Measure a removal with `--diff`.
 */
const FAMILIES = [
