 ...
 *
 * Subclasses supply an `EntitySelectSpec` (from `entitySelectConfig`) and a
 * template declaring its own `<nz-option>`s inside `<nz-select>`: nz-select
 * binds options at template declaration, so they can't be projected through a
 * wrapper (projected-options.spec.ts). Results, loading and messages are
 * signals; nothing here calls detectChanges.
 */
@Directive()
...
> extends CvcFieldBase<CvcEntitySelectValue, FieldTypeConfig<P>> {
  ...
  private readonly typeaheadResource = rxResource({
    ...
    // network-only: cache-first replayed the listing cached when the dropdown
    // first opened, so an entity quick-added since never appeared in it
    stream: ({ params }) =>
