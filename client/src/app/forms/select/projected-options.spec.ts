...
// declaration, not DOM projection: <nz-option>s re-projected through a
// wrapper's <ng-content> are never seen, and the dropdown renders empty. So
// the select chrome is an attribute directive, with <nz-option> declared
// directly inside <nz-select> in each field template.
//
// The tests below characterize that shape: dynamic @for over a signal,
// nzCustomContent option bodies with closure context, nzServerSearch, and
// nzCustomTemplate selected items.
