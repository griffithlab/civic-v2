...
export class CvcSourceQuickAddForm implements OnInit, OnChanges {
   ...
   * asynchronously, so a read-back yields `citation: null` and the tag reads
   * "PubMed: 5540" until the next page load. This writes the citation the
   * form already looked up, through the select's generated fragment. Delete
   * once addRemoteCitation returns a populated Source.
   */
