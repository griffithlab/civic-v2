 ...
 *
 * The Feature comes from a sibling field via form state, a parameter gate
 * rather than an entity-type gate, so this extends the plain base. The tag
 * query resolves to GeneVariant, FactorVariant, FusionVariant, RegionVariant
 * or plain Variant, so the tag typename is resolved per result; the
 * typeahead only returns plain Variant.
 */
@Component({
