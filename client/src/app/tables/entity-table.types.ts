...
/**
 * The column model. The discriminant covers only the cell's contents; layout
 * (width, align, fixed) lives on the column itself, so one `<th>` and one
 * `<td>` serve every column and the union is narrowed once, where it varies.
 */
