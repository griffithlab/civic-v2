 ...
 *
 * It reports rather than acts: the host owns the QueryRef, so one pipeline can
 * serialize refetches and fetchMores instead of racing them.
 *
 * 1. Bottom detection reads the current offset, not successive offsets, so
 *    dragging the scrollbar to the end in one motion still fires.
 * 2. A cursor already in flight isn't re-requested, so that looser trigger
 *    can't burst duplicate pages.
 */
...
export class CvcTableScrollDirective {
  ...
  /**
   * Keeps the viewport's measurement in step with its container's size.
   *
   * CDK measures the viewport once, on first render, before the auto-height
   * directives have sized the container, and never re-measures. Observing the
   * element covers every resize: auto-height settling, a drawer, the window,
   * column visibility. The re-measure is coalesced into an animation frame
   * because `checkViewportSize()` can itself resize the element and re-enter
   * the observer.
   */
