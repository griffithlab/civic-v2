...
import { TherapyPopoverGQL, Maybe, TherapyPopover} from "@app/generated/civic.apollo";
...
export class CvcTherapyPopoverComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.therapy$ = this.gql.watch({ therapyId: this.therapyId })
      .valueChanges
      .pipe(map(({ data }) => data?.therapyPopover),
        filter(isNonNulled));
  }
}
