...
import { RevisionBrowseFieldsFragment, RevisionConnection, RevisionsBrowseGQL, RevisionsBrowseQuery, RevisionsBrowseQueryVariables, Maybe, PageInfo, Revision } from '@app/generated/civic.apollo';
...
export class CvcRevisionsTableComponent implements OnInit {
  ...
  revisions$?: Observable<Maybe<RevisionBrowseFieldsFragment>[]>
  ...
  constructor(private gql: RevisionsBrowseGQL, private cdr: ChangeDetectorRef) {
  ...
  ngOnInit() {
    ...
    this.connection$ = this.result$
      .pipe(pluck('data', 'revisions'),
        filter(isNonNulled)) as Observable<RevisionConnection>;
...
/*     this.sortChange$
  ...
  trackByIndex(_: number, data: RevisionBrowseFieldsFragment): number {
    return data.id;
  }
