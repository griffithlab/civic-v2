...
/**
 * Golden specs: a regression guard for the evidence manager that keeps
 * passing when its template is replaced. Address the table through the
 * `data-testid` contract, never its DOM shape.
 *
 * Unauthenticated on purpose, so it runs on a fresh checkout:
 * /assertions/add renders its form without a session. /variant-groups/add
 * requires a login, so the variant manager's goldens live in a separate,
...
/**
 * Known failing: reset pushes null into every column's `filter.changes`, so
 * the rows repopulate, but it never clears `col.filter.options[0].value`, the
 * backing store for the filter input's [cvcModel]. The table and its filter
 * boxes disagree, and the reset button reads as inert.
 *
 * `test.fail()` keeps the suite green while the bug exists and turns it red
 * once the bug is fixed; delete the annotation then.
 */
