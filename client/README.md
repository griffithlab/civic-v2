...
Tests run with Vitest through the Angular CLI's unit-test builder (`src/test-setup.ts` is the setup file). The suite is a smoke layer — app bootstrap, route rendering, and the query builder (`src/app/*.smoke.spec.ts`) — that catches breakage during dependency upgrades.
